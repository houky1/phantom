...
!
   ...
   ! save from previous call (of get_gw_force) -- NOT used by L1_point
   ! any more (see hpr_l1_context below); kept here only because other
   ! parts of the original analysis_stripping_tools.f90 this module was
   ! ported from used them.
   real, save, public :: evector_old(3)
   real, save, public :: time_old(2)
   real, save, public :: omega_old(3)

   ! Bundles everything the L1 potential/force evaluation needs, so it
   ! can be threaded explicitly through golden_section_search_method's
   ! and newton_method's func(point,...) callback argument instead of
   ! being read from module-level state. Never crosses the module
   ! boundary -- L1_point's own public signature still takes plain
   ! xyzh/particlemass/npart/axis/com/omega arguments; this type is
   ! purely an internal device for satisfying the fixed callback
   ! interface (which has no room for extra arguments of its own).
   type :: hpr_l1_context
...
!-----------------------------------------------------------------------
   subroutine L1_point(method, xyzh, particlemass, npart, axis, com_in, omega, rmax, L1_proj, L1)
      ...
      if(method == 1) then
         ...
         ! Golden section search requires the objective to be unimodal
         ! on [p1,p2]. The raw system extent [-rmax,rmax] straddles the
         ! two stars themselves, where the (unsoftened) point-mass
         ! potential plunges sharply near each star -- not unimodal.
         ! Coarsely scan each half (relative to the COM) for the point
         ! of most negative potential, as an approximate location of
         ! each star, and bracket the search strictly between them,
         ! where the Roche potential has a single interior maximum (L1).
...
!-----------------------------------------------------------------------
   subroutine bracket_between_stars(ctx, rmax, p1, p2)
      type(hpr_l1_context), intent(in)  :: ctx
      real,                  intent(in)  :: rmax
      ...
      if (p2 - p1 < 1.e-6*rmax) then
         ...
         ! pull the bracket inward from each star's approximate
         ! location, so the search doesn't sit right at the edge of a
         ! dense particle cloud where curvature is steep and the
         ! min/max classification probe (and golden section itself)
         ! can be dominated by local noise rather than the large-scale
         ! shape of the potential between the two stars
         margin = 0.15*(p2 - p1)
         p1 = p1 + margin
         p2 = p2 - margin
      endif
...
!-----------------------------------------------------------------------
   real function golden_section_search_method(a, b,&
      ...
      ! Determine whether the interior extremum is a min or a max by
      ! comparing the left edge to the bracket MIDPOINT, not a+eps.
      ! eps is the final convergence tolerance (typically ~1e-7) and is
      ! completely unrelated to the scale of the bracket [a,b]; probing
      ! a tiny absolute step away from `a` is only meaningful if `a`
      ! itself sits in a smooth region. If `a` happens to sit near a
      ! region of steep local curvature (e.g. close to a point mass in
      ! the L1 search), that tiny step drowns in local noise/curvature
      ! and can misclassify max as min. The midpoint reflects the
      ! large-scale shape of the whole interval instead.
      call func(a, ctx, fy0)
